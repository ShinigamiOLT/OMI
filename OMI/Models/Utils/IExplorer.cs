...
namespace OMI.Models.Utils
{
    ...
    {
        MemoryStream DTCatalogosExport<T>(List<T> results, List<DTColumn> columns);
    }
    public class PdfExporter : IExporter
    {
        public MemoryStream DTCatalogosExport<T>(List<T> results, List<DTColumn> columns)
        {
            var document = new Document(PageSize.LETTER, 20f, 20f, 20f, 20f);
            var stream = new MemoryStream();

            try
            {
                var colsNames = columns
                    .Where(c => !string.IsNullOrWhiteSpace(c.Data))
                    .Select(c => c.Data.Trim()).ToList();

                PdfWriter.GetInstance(document, stream);
                document.Open();

                PdfPTable table = new PdfPTable(colsNames.Count)
                {
                ...
                results.ForEach(result =>
                {
                    ...
                    if (header)
                    {
                        colsNames.ForEach(c =>
                        {
                            var property = result.GetType().GetProperty(c);
                            if (property == null) return;

                            var displayAttr =
                                property.GetCustomAttributes(typeof(DisplayNameAttribute), true).SingleOrDefault();
                            var headerValue = displayAttr != null
                                ? displayAttr.GetType().GetProperty("DisplayName").GetValue(displayAttr, null)
                                : c;
                            table.AddCell(headerValue.ToString());
                        });
                        header = false;
                        table.CompleteRow();
                    }
                    ...
                    colsNames.ForEach(c =>
                    {
                        ...
                        table.AddCell(resultValue.ToString());
                    });
                    table.CompleteRow();
                });
                document.Add(table);
    ...
    public class ExcelExporter : IExporter
    {
        public MemoryStream DTCatalogosExport<T>(List<T> results, List<DTColumn> columns)
        {
            ...
            // create sheet
            var sh = (XSSFSheet)wb.CreateSheet("Hoja1");
            ...
            results.ForEach(result =>
            {
                ...
                if (i == 1)
                {
                    colsNames.ForEach(c =>
                    {
                        ...
                        var displayAttr = property.GetCustomAttributes(typeof(DisplayNameAttribute), true).SingleOrDefault();
                        var headerValue = displayAttr != null ? displayAttr.GetType().GetProperty("DisplayName").GetValue(displayAttr, null) : c;
    ...
    public class DTParameters
    {
        ...
        /// <summary>
        /// A list defining all columns in the table.
        /// </summary>
